...
if [ "${BUILD_COMMAND}" = 'build' ]; then
  ...
  export TEST_TO_RUN=${TEST_TO_RUN:-'-Dts.multinode -Dtest=SecuredBeanClientInterceptorTestCase,DatabaseTimerServiceMultiNodeExecutionDisabledTestCase,DatabaseTimerServiceMultiNodeTestCase'}
