...
if [ ! -f "glassfish.zip" ]
then 
  echo "failed: glassfish.zip is missing, cannot run standalone SAAJ TCK tests"
  exit 1 
fi
unzip -o glassfish.zip > /dev/null && rm -f glassfish.zip

echo "glassfish version:"
cat glassfish.version

echo "glassfish checksum:"
cat glassfish.cksum

# get tck
curl -k https://download.eclipse.org/jakartaee/soap-attachments/1.4/jakarta-soap-tck-1.4.0.zip -o jakarta-soap-tck-1.4.0.zip
unzip -o jakarta-soap-tck-1.4.0.zip > /dev/null && rm -f jakarta-soap-tck-1.4.0.zip

mv soap-tck javaeetck 
...
if [ ! -d "tckmods" ]
then 
  echo "failed: tckmods folder is missing, cannot run standalone SAAJ TCK tests"
  exit 1 
fi
cp -r tckmods $JEETCK_MODS
cd $JEETCK_MODS

# sed "s%jaxwstck.jar%saajtck.jar%" -i $JEETCK_MODS/bin/ts.jte
# sed "s%com.sun.ts.lib.deliverable.jaxws.JAXWSDeliverable%com.sun.ts.lib.deliverable.tck.TCKDeliverable%" -i $JEETCK_MODS/bin/ts.jte

pwd
ls $JEETCK_MODS/bin
echo jboss.home=$JBOSS_HOME > ant.properties
echo ts.home=$TS_HOME >> ant.properties
echo jeetck.mods=$JEETCK_MODS >> ant.properties
